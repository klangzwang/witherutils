...
public class SmallToggleButton<U extends WUTScreen<?>, T extends AbstractWidget> extends AbstractWidget {
    ...
    public void renderWidget(GuiGraphics gg, int pMouseX, int pMouseY, float pPartialTick)
    {
    	...
    	if(getter.get() == false)
    		gg.blit(WitherUtils.loc("textures/gui/" + res + "_off.png"), leftPos, topPos, 0, 0, size, size, size, size);
    	else
    		gg.blit(WitherUtils.loc("textures/gui/" + res + "_on.png"), leftPos, topPos, 0, 0, size, size, size, size);
