...
public class FilterItem extends FilterBase {
    ...
    public boolean testItem(ItemStack stack) {
        ...
        if (isTagMode()) {
            ...
            match = fuzzyMatch ? ItemStack.isSameItem(filterStack, stack) : ItemStack.isSameItem(filterStack, stack) && (filterStack.getCount() == stack.getCount() || !matchCount);
        }
    ...
    public TagKey<Item> getTag() {
        if (tagCache == null) {
            tagCache = ItemTags.create(ResourceLocation.withDefaultNamespace(tagString));
        }
        return tagCache;
    }
