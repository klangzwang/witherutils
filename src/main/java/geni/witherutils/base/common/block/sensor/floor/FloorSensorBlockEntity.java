...
public class FloorSensorBlockEntity extends WitherMachineBlockEntity implements MenuProvider {
	...
	{
		super(WUTBlockEntityTypes.FLOORSENSOR.get(), pos, state);
		addDataSlot(NetworkDataSlot.INT.create(this::getScanWidth, p -> scanWidth = p));
		addDataSlot(NetworkDataSlot.INT.create(this::getTrigger, p -> trigger = p));
	}
