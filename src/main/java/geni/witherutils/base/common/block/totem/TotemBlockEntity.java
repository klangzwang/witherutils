...
public class TotemBlockEntity extends WitherMachineFakeBlockEntity implements MenuProvider {
    ...
    {
        super(WUTEntities.TOTEM.get(), worldPosition, blockState);
	...
	private void collectEntity(@Nonnull LivingEntity entity)
	{
		...
		{
			if(handler.getValue() == TOTEMTYPE.getItemStack(inventory).getItem())
			{
	...
	{
		return new AABB(getBlockPos().offset(-scaleX, 0, -scaleZ), getBlockPos().offset(scaleX +1, scaleY, scaleZ +1));
	}
    ...
    public static class EventHandler
    {
		@SubscribeEvent
		@OnlyIn(Dist.CLIENT)
        public void spawnParticle(final TickEvent.ClientTickEvent event)
        {
