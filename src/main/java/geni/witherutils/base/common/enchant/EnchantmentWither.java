...
public abstract class EnchantmentWither extends Enchantment {

    protected EnchantmentWither(Rarity rarityIn, EnchantmentCategory typeIn, EquipmentSlot[] slots)
    {
    ...
    {
        if (stack.isEmpty() == false && EnchantmentHelper.getEnchantments(stack).containsKey(this) && stack.getItem() != Items.ENCHANTED_BOOK)
        {
    ...
    protected int getCurrentArmorLevelSlot(LivingEntity player, EquipmentSlot type)
    {
        ...
        int level = 0;
        if (armor.isEmpty() == false && EnchantmentHelper.getEnchantments(armor) != null && EnchantmentHelper.getEnchantments(armor).containsKey(this))
        {
    ...
    {
        EquipmentSlot[] armors = new EquipmentSlot[] { EquipmentSlot.CHEST, EquipmentSlot.FEET, EquipmentSlot.HEAD, EquipmentSlot.LEGS };
        int level = 0;
