...
public class SavedUpgrades {
    private static final Codec<WUTUpgrade> UPGRADE_CODEC
            = ResourceLocation.CODEC.xmap(id -> ApplicableUpgradesDB.getInstance().getUpgradeById(id), WUTUpgrade::getId);
    private static final StreamCodec<ByteBuf,WUTUpgrade> UPGRADE_STREAM_CODEC
            = ResourceLocation.STREAM_CODEC.map(id -> ApplicableUpgradesDB.getInstance().getUpgradeById(id), WUTUpgrade::getId);
    ...
    private final Map<WUTUpgrade,Integer> map;

    private SavedUpgrades(ItemContainerContents contents, Map<WUTUpgrade,Integer> map) {
        this.contents = contents;
        this.map = map;
    }

    public static SavedUpgrades fromItemHandler(IItemHandler upgradeHandler) {
        ImmutableMap.Builder<WUTUpgrade,Integer> builder = ImmutableMap.builder();
    ...
    public int getUpgradeCount(WUTUpgrade upgrade) {
        return map.getOrDefault(upgrade, 0);
    }

    public Map<WUTUpgrade, Integer> getUpgradeMap() {
