...
public class MachineInventoryLayout {
    ...
    public boolean supportsSoulBank()
    {
        return soulBankSlot >= 0;
    }
    ...
    public static class Builder
    {
        ...
        public Builder soulbank()
        {
            if (soulBankSlot >= 0)
            {
                throw new IllegalStateException("A machine inventory may not have more than one soulbank slot!");
            }
            slot(slot -> slot.guiInsert().guiExtract().filter((i, s) -> SoulBankUtil.isSoulBank(s)).stackLimit(1));
            soulBankSlot = slots.size() - 1;
            return this;
        }
