...
public interface FluidItemInteractive {
	...
	default boolean handleFluidItemInteraction(Player player, InteractionHand hand, ItemStack itemStack,
			FluidTankUser machine, TankAccess tankAccess) {
		...
		if (fluidHandlerItem.isPresent()) {
			...
			if (tankAccess.isEmpty(machine)) {
				...
				fluidInItem = handler.drain(new FluidStack(tankAccess.getFluid(machine).getFluid(), Integer.MAX_VALUE),
